...
class BaseDataset(Dataset): #TODO: image_dir is still single path
    def __init__(self, datasets_path_list, image_dir, image_size_dims = [720, 576],  neighborhood_size = 32, neighborhood_radius = 32, grid_radius = 4, grid_angle = 45):
        super().__init__()                
        ...
        person_data = []
        group_data = []
        for i, dataset_path in datasets_path_list:            
            obs = np.load(dataset_path + "/obs.npy") 
            pred = np.load(dataset_path + "/pred.npy")
